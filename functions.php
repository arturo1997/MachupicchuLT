...
function widgets_footer() {
    ...
    register_sidebar(array(
        'name'          => 'menu idioma',
        'id'            => 'menu-idioma',
        'description'   => 'Esta es una área de widget personalizada para el cambio de idioma.',
        'before_widget' => '<div id="%1$s" class="widget %2$s">',
        'after_widget'  => '</div>',
        'before_title'  => '<h5 class="widget-title">',
        'after_title'   => '</h5>',
...
function registrar_menu_footer() {
    register_nav_menu('menu_footer', 'Menú de Footer');
}
...
/**
 * SECION PARA NUMEROS Y CORREOS
 * 
 */
function agregar_seccion_personalizada($wp_customize) {
    // Sección para Números de Teléfono
    $wp_customize->add_section('seccion_numeros', array(
        'title'    => __('Números de Teléfono', 'textdomain'),
        'priority' => 120,
    ));

    // Campos para Números de Teléfono
    for ($i = 1; $i <= 5; $i++) {
        $wp_customize->add_setting('telefono_' . $i, array(
            'default'           => '',
            'sanitize_callback' => 'sanitize_text_field',
        ));

        $wp_customize->add_control('telefono_' . $i, array(
            'label'    => __('Número de Teléfono ' . $i, 'textdomain'),
            'section'  => 'seccion_numeros',
            'type'     => 'text',
        ));
    }

    // Sección para Correos Electrónicos
    $wp_customize->add_section('seccion_correos', array(
        'title'    => __('Correos Electrónicos', 'textdomain'),
        'priority' => 121,
    ));

    // Campos para Correos Electrónicos
    for ($i = 1; $i <= 3; $i++) {
        $wp_customize->add_setting('correo_' . $i, array(
            'default'           => '',
            'sanitize_callback' => 'sanitize_email',
        ));

        $wp_customize->add_control('correo_' . $i, array(
            'label'    => __('Correo Electrónico ' . $i, 'textdomain'),
            'section'  => 'seccion_correos',
            'type'     => 'email',
        ));
    }

    // Botón para agregar más
    $wp_customize->add_setting('boton_agregar_mas', array(
        'default' => '',
    ));

    $wp_customize->add_control('boton_agregar_mas', array(
        'label'    => __('Botón para Agregar Más', 'textdomain'),
        'section'  => 'seccion_correos',
        'settings' => 'boton_agregar_mas',
        'type'     => 'button',
    ));
}

add_action('customize_register', 'agregar_seccion_personalizada');
